...
export default function TeamEdit({ member }: Props) {
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        post(`/admin/team/${member.id}`, {
            forceFormData: true,
            onSuccess: () => toast.success('Team member updated.'),
        });
    }

    const currentPhotoUrl = photoPreview ?? (member.photo ? `/storage/${member.photo}` : undefined);
                    ...
                    <div className="grid gap-2">
                        ...
                        <div className="flex items-center gap-5">
                            <Avatar className="h-20 w-20">
                                <AvatarImage src={currentPhotoUrl} alt={member.name} className="object-cover" />
